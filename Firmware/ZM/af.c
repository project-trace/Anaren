...
*/
moduleResult_t afRegisterApplication(struct applicationConfiguration ac)
{
#ifdef AF_VERBOSE
    ...
    {
        zmBuf[bufferIndex++] = ac.bindingInputClusters[i] & 0xFF;
        zmBuf[bufferIndex++] = ac.bindingInputClusters[i] >> 8;
    }
    zmBuf[bufferIndex++] = ac.numberOfBindingOutputClusters;
    for (i = 0; i<ac.numberOfBindingOutputClusters; i++)
    {
        zmBuf[bufferIndex++] = ac.bindingOutputClusters[i] & 0xFF;
        zmBuf[bufferIndex++] = ac.bindingOutputClusters[i] >> 8;
    }
    zmBuf[0] = bufferIndex;
...
#define AF_DATA_CONFIRM_TIMEOUT 2
        ...
        uint16_t totalMessageIndex = 0;  //index in the Module data buffer
...
moduleResult_t retrieveExtendedMessage(uint8_t* ts, uint16_t length, uint8_t* destinationPtr)
{
    ...
    printHexBytes(ts, 4);
#endif
    uint16_t totalMessageIndex = 0;  //index in the destinationPtr buffer
    uint8_t timestamp[4];
    memcpy(timestamp, ts, 4);    
    
    while (length > 0)  //while there are remaining bytes
    {
        uint8_t bytesToGet = 0;
        if (length > MAXIMUM_AF_DATA_RETRIEVE_PAYLOAD_LENGTH)       //get the most we can
            bytesToGet = MAXIMUM_AF_DATA_RETRIEVE_PAYLOAD_LENGTH;
        else
            bytesToGet = length;
#ifdef AF_VERBOSE          
